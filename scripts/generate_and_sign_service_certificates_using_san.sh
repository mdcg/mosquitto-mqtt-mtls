...
openssl x509 -req -in "$service_filename.csr" -CA "$ca_certificate_filename" -CAkey "$ca_key_filename" -CAcreateserial -out "$service_filename.crt" -days 360 -extfile v3.ext

echo "Chave e certificado CA gerados e assinados com sucesso! (Usando SAN)"
